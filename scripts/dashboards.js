(function(root){
 ...
 function portfolio(list){
  ...
  host('esf-kpis',metrics([{label:'Contrato + aditivos',value:money(s.contract),sub:missing?`${missing} obras sem contrato valorizado`:`${list.length} obras neste recorte`,tone:'blue'},
   {label:'Pago dentro da obra',value:money(s.paid),sub:s.legacy?money(s.legacy)+' de saldo histórico sem lançamentos':'Pagamentos registrados dentro da obra',tone:'green'},
   {label:'Lançamentos pendentes na obra',value:money(s.open),sub:'Não inclui pagamentos fora da obra',tone:'amber'},
  ...
  host('esf-alert',`<div class="dash-shell">${context(`${list.length} obras no filtro atual`)}<div class="dash-columns">${section('Contratos e exposição',table(['Obra',{label:'Contratado',money:true},{label:'Pago',money:true},{label:'Pendente',money:true},{label:'Investimento livre',money:true}],rows.map(r=>[cell(mark(r.o.marca)+button(r.o.nome,'project',{id:r.o.id}),r.o.nome),cash(r.contract),cash(r.paid),cash(r.open),cash(r.freeInvestment)]),'works'),'Investimento livre considera obrigações contratuais ainda não lançadas. Sem investimento definido, o saldo não é calculado.',download('works'))}${section('Prioridades de pagamento',bars([{label:'Vencidos',value:s.ledger.overdueValue,tone:'red',sub:s.ledger.overdue.length+' lançamentos'},{label:'Vencem hoje',value:s.ledger.todayValue,tone:'amber',sub:s.ledger.dueToday.length+' lançamentos'},{label:'Próximos 30 dias',value:D.sum(s.ledger.next30,r=>r.value),tone:'blue',sub:s.ledger.next30.length+' lançamentos'},{label:'Sem vencimento',value:s.ledger.undatedValue,tone:'neutral',sub:s.ledger.undated.length+' lançamentos'}]))}</div></div>`);
 }
 function project(){
  ...
  host('o-kpis',metrics([{label:'Contrato + aditivos',value:money(s.contract),sub:(s.contracts.source==='builders'?'Cadastro das construtoras':'Cadastro geral da obra')+' · '+money(s.add)+' aditivos',tone:'blue',action:'project-tab',args:{tab:'contr'}},{label:'Pago dentro da obra',value:money(s.paid),sub:'Todos os pagamentos do escopo obra',tone:'green'},{label:'Lançamentos pendentes na obra',value:money(s.open),sub:l.open.filter(r=>r.scope==='obra').length+' lançamentos',tone:'amber',action:'project-payments',args:{scope:'obra',status:'pend'}},{label:'Contratos complementares',value:money(s.contracts.supplierTotal),sub:'Separados dos contratos das construtoras'}]));
  const planned=D.amount(o.investimento_disponivel),ceiling=D.amount(o.teto_escola);
  html+=section('Limites de planejamento',metrics([{label:'Investimento previsto',value:planned>0?money(planned):'Não definido',sub:'Previsão cadastrada, não é aporte recebido'},{label:'Investimento livre após compromissos',value:money(s.freeInvestment),sub:'Desconta pagos e obrigações contratuais ainda a pagar',tone:s.freeInvestment<0?'red':'green'},{label:'Teto do caixa da escola',value:ceiling>0?money(ceiling):'Não definido',sub:'Limite dos pagamentos fora da obra'},{label:'Caixa livre após compromissos',value:money(s.freeSchool),sub:'Desconta pagos e obrigações atribuídas à escola',tone:s.freeSchool<0?'red':'green'}]),'',AccessControl.canEdit(o.esfera||'nova')?`<div class="dash-tools">${button('Investimento','project-settings')}${button('Teto escolar','school-ceiling')}${button('Aditivos','project-additions')}</div>`:'' );
  const contractorRows=s.contracts.rows.map(c=>[c.name+(c.supplier?' · '+c.supplier:''),c.kind==='supplier'?'Complementar':'Construtora',cash(c.total),cash(c.paid),cash(c.open),cash(c.unbilled),cash(c.balance)]);
  ...
  html+=`${section('Contrato e construtoras',contractorRows.length?table(['Contrato','Tipo',{label:'Total com aditivos',money:true},{label:'Pago vinculado',money:true},{label:'Pendente lançado',money:true},{label:'Ainda não lançado',money:true},{label:'Saldo contratual',money:true}],contractorRows):note(o.constr||'Construtora não informada'),'Cada contrato aparece uma vez. Parcelas pendentes já fazem parte do saldo contratual.',button('Contratos','project-tab',{tab:'contr'}))}${section('Andamento operacional',phaseBody,'Etapas cadastradas da obra',button('Fases da obra','project-tab',{tab:'fases'}))}`;
  html+=section('Caixa fora do contrato',metrics([{label:'Pago fora da obra',value:money(s.paidExtra),sub:'Não reduz o contrato principal'},{label:'Pendente fora da obra',value:money(s.openExtra),sub:'Compromissos do caixa da escola'},{label:'Pago sem vínculo contratual',value:money(s.contracts.unlinked.paidValue),sub:'Não atribuído automaticamente a uma construtora'}]),'',button('Pagamentos da escola','project-payments',{scope:'extra'}));
  if(s.contracts.unlinked.paidValue||s.contracts.conflicts.length)html+=note('Conferência de vínculos: '+money(s.contracts.unlinked.paidValue)+' pagos sem contrato identificado. '+s.contracts.conflicts.length+' lançamentos com dois vínculos; o contrato complementar tem prioridade. Valores não foram redistribuídos.','amber');
  html+=section('Obrigações ainda a pagar',metrics([{label:'Pelo investimento',value:money(s.contracts.investmentExposure),sub:'Saldo dos contratos e pendências avulsas, sem duplicação'},{label:'Pela escola',value:money(s.contracts.schoolExposure),sub:'Respeita a divisão cadastrada em cada contrato'}]));
