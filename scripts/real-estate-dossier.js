(function(root){
  ...
  function metricsHtml(groups,expense='all'){const s=totals(groups),label=expense==='all'?'Total nominal dos TRs':'Subtotal nominal · '+expenseLabel(expense);return `<div class="re-dossier-metrics"><div><span class="lab">${esc(label)}</span><strong class="val">${money(s.total)}</strong></div><span>${s.count} lançamento(s)</span><span>${s.review} a conciliar</span><span>${s.provisional} previsão(ões)</span></div><p class="re-dossier-caption">Total nominal não comprova pagamento. Sem previsões, cancelados, divergências ou rateios pendentes na soma.</p>`;}
