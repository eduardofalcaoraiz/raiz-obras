(function(root){
  ...
  function metricsHtml(groups,expense='all'){const s=totals(groups),label=expense==='all'?'Total nominal dos TRs':'Subtotal nominal · '+expenseLabel(expense);return `<div class="dash-metrics"><div class="dash-metric blue"><span>${esc(label)}</span><strong>${money(s.total)}</strong><small>Valores registrados e conciliados</small></div><div class="dash-metric"><span>Lançamentos somados</span><strong>${s.count}</strong><small>No recorte selecionado</small></div><div class="dash-metric amber"><span>A conciliar</span><strong>${s.review}</strong><small>Conferir valores e vínculos</small></div><div class="dash-metric"><span>Previsões</span><strong>${s.provisional}</strong><small>Fora do total nominal</small></div></div><p class="re-dossier-caption">Total nominal não comprova pagamento. Sem previsões, cancelados, divergências ou rateios pendentes na soma.</p>`;}
