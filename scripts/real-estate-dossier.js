(function(root){
  ...
  const expenseTypes=[['all','Visão geral'],['rent','Aluguel'],['iptu','Cotas de IPTU'],['condo','Condomínio'],['fire','Taxa de incêndio'],['insurance','Seguros'],['utilities','Água, energia e gás'],['other','Outros encargos'],['unclassified','A classificar']];
  const expenseLabel=key=>expenseTypes.find(([k])=>k===key)?.[1]||'A classificar';
  function expenseType(value){const t=norm(value);if(t==='aluguel')return 'rent';if(['iptu','parcelamento iptu','refis iptu'].includes(t))return 'iptu';if(t==='condominio')return 'condo';if(['taxa de incendio','incendio','funesbom'].includes(t))return 'fire';if(['seguro','seguro patrimonial','seguro incendio'].includes(t))return 'insurance';if(['agua','esgoto','energia','luz','gas','gas gastroservice'].includes(t))return 'utilities';return !t||['financeiro','atipico'].includes(t)?'unclassified':'other';}
  ...
  function metricsHtml(groups,expense='all'){const s=totals(groups),label=expense==='all'?'Total nominal dos TRs':'Subtotal nominal · '+expenseLabel(expense);return `<div class="record-totals"><span>${esc(label)}: <strong>${money(s.total)}</strong></span><span>Lançamentos somados: ${groups.filter(g=>g.countable).length}</span><span>A conciliar: ${s.review}</span><span>Previsões: ${s.provisional}</span></div><p class="re-dossier-caption">Total nominal não comprova pagamento. Sem previsões, cancelados, divergências ou rateios pendentes na soma.</p>`;}
  ...
  function tab(button,name){const el=button.closest('.re-dossier'),s=caches.get(el);if(!s||!tabs.some(([key])=>key===name)||((s.loading||s.error)&&name!=='contrato'))return;s.tab=name;s.page=0;if(name==='financeiro')s.type='';draw(el);}
