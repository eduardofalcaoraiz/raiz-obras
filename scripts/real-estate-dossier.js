(function(root){
  ...
  function metricsHtml(groups,expense='all'){const s=totals(groups),label=expense==='all'?'Total nominal dos TRs':'Subtotal nominal · '+expenseLabel(expense);return `<div class="kpis re-dossier-metrics"><div class="kpi"><div class="lab">${esc(label)}</div><div class="val">${money(s.total)}</div><div class="sub">${groups.filter(g=>g.countable).length} TRs sem divergência no recorte</div></div><div class="kpi"><div class="lab">Lançamentos</div><div class="val">${s.count}</div><div class="sub">No recorte selecionado</div></div><div class="kpi re-metric-review"><div class="lab">Conciliação / rateio</div><div class="val">${s.review}</div><div class="sub">Não somados ao total</div></div><div class="kpi re-metric-forecast"><div class="lab">Previsões sem TR</div><div class="val">${s.provisional}</div><div class="sub">Referências das planilhas</div></div></div><p class="re-dossier-caption">Total nominal não comprova pagamento. Previsões, cancelados, divergências e rateios pendentes não entram na soma.</p>`;}
  ...
  async function open(card,id,force=false){if(card.tagName==='DETAILS'&&!card.open)return;const el=card.querySelector('.re-dossier');if(!el)return;let state=caches.get(el);if(state?.loading)return;if(state&&!force){draw(el);return;}
    state={id,loading:true,tab:state?.tab||'financeiro',expense:state?.expense||'all',year:state?.year??'2026',type:state?.type||'',query:state?.query||'',month:state?.month||'',supplier:state?.supplier||'',state:state?.state||'',page:0,hist:[],tickets:[],docs:[]};caches.set(el,state);
  ...
  function toolbar(s,groups){
    ...
    if(s.query||(!docs&&(s.type||s.supplier||s.month||s.state)))fields+='<button class="btn btn-ghost btn-sm re-filter-clear" title="Limpar filtros" aria-label="Limpar filtros" onclick="RealEstateDossier.clear(this)">'+icon('x')+'</button>';
  ...
  function filter(input,key,value){const el=input.closest('.re-dossier'),s=caches.get(el);if(!s||!['query','year','type','month','supplier','state'].includes(key))return;const position=input.selectionStart;s[key]=value;s.page=0;if(key==='year')s.month='';draw(el);if(key==='query'){const next=el.querySelector('[data-focus=search]');next.focus();next.setSelectionRange(position,position);}else if(key==='state'){el.querySelector('.chip.active')?.focus();}else el.querySelector(`[data-filter="${key}"]`)?.focus();}
  function clear(button){const el=button.closest('.re-dossier'),s=caches.get(el);if(!s)return;Object.assign(s,{type:'',query:'',month:'',supplier:'',state:'',page:0});draw(el);el.querySelector('[data-focus=search]')?.focus();}
  ...
  root.RealEstateDossier={section,open,tab,key,filter,clear,page,expand,expense,expenseParts,expenseGroups,groupHistory,selectGroups,totals,documents,financeHtml,ticketRows,paginate};
